...
function evolve(tensor_problem::TensorProblem, T_final::Float64, schedule_x::Function, schedule_z::Function, catalyst_schedule_x::Function, catalyst_schedule_z::Function; rtol=1e-4, atol=1e-6)
    ...
    function mf_eom(dS, S, _, t)
        ...
        magnetization_z = magnetization(S[3, :], ztensor)
        catalyst_x = magnetization(S[1, :], catalyst_xtensor)
        catalyst_z = magnetization(S[3, :], catalyst_ztensor)

        dnx(i) = -2 * (schedule_z(t) * magnetization_z[i] + catalyst_schedule_z(t) * catalyst_z[i]) * S[2, i]
        dny(i) = -2 * (schedule_x(t) * magnetization_x[i] + catalyst_schedule_x(t) * catalyst_x[i]) * S[3, i] + 2 * (schedule_z(t) * magnetization_z[i] + catalyst_schedule_z(t) * catalyst_z[i]) * S[1, i]
        dnz(i) =  2 * (schedule_x(t) * magnetization_x[i] + catalyst_schedule_x(t) * catalyst_x[i]) * S[2, i]
        dS .= reduce(hcat, [[dnx(i), dny(i), dnz(i)] for i in 1:size(S)[2]])
    end
